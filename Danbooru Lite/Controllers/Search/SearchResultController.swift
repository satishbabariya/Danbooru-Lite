...
class SearchResultController: BooruController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        if let tag = tag {
            title = tag.replacingOccurrences(of: "+", with: " ").camelcase
        }
    ...
    fileprivate func postListRequest() {
        queue.addOperation { [weak self] in
            ...
            RESTClient.posts(limit: self!.pageLimit, page: 1, tags: self!.tag)
                .request()
                // .debug()
    ...
    fileprivate func loadMorePostRequest(page: Int) {
        queue.addOperation { [weak self] in
            ...
            RESTClient.posts(limit: self!.pageLimit, page: page, tags: self!.tag)
                .request()
                // .debug()
                .subscribe({ [weak self] event in
                    if self == nil {
