...
class PageView: UIScrollView {
    ...
    func setImage() {
        ...
        imageView.kf.setImage(with: image.imageURL, placeholder: nil, options: nil, progressBlock: nil) { [weak self] (_, _, _, _) in
            if self == nil{
                return
            }
...
//    }
    ...
    func configure() {
        ...
        activityIndicator.color = GradientColor(gradientStyle: UIGradientStyle.leftToRight, frame: .init(x: 0, y: 0, width: Application.device.iPad ? 70.0 : 60.0, height: Application.device.iPad ? 70.0 : 60.0), colors: Application.Theme.color.gradient)
        addSubview(activityIndicator)
        activityIndicator.centerXAnchor.constraint(equalTo: self.centerXAnchor).isActive = true
        activityIndicator.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
        activityIndicator.widthAnchor.constraint(equalToConstant: Application.device.iPad ? 70.0 : 60.0).isActive = true
        activityIndicator.heightAnchor.constraint(equalToConstant: Application.device.iPad ? 70.0 : 60.0).isActive = true
    }
